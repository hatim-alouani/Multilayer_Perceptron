...
# bonus: history of the metrics obtained during training
loss_history = []
val_loss_history = []
acc_history = []
val_acc_history = []
...
for epoch in range(epochs):
    ...
    train_loss = helpers.loss(y_train, out)
    val_loss = helpers.loss(y_valid, val_out)
...
# save model (topology + weights + preprocessing params needed for prediction)
...
# save training history (bonus), as json instead of a binary numpy file
history = {
...
subprocess.run([sys.executable, 'plot.py'])
